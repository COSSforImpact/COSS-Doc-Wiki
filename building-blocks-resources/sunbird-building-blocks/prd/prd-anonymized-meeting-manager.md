...
# PRD---Anonymized-meeting-manager

* [Brief](prd-anonymized-meeting-manager.md#brief)
* [Problem Statement](prd-anonymized-meeting-manager.md#problem-statement)
* [Suggested Solution](prd-anonymized-meeting-manager.md#suggested-solution)
* [Alternatives considered](prd-anonymized-meeting-manager.md#alternatives-considered)
...
2. When a Github user wishes to start a meeting, they create an event.
3. In the event, they can add:
   1. Github IDs of the intended recipients
   2. See potential dates/times that work for the recipients (if the calendars are compatible/have API for this)
   3. Post the github main thread/subthread where they want updates/link access to this event.
4. The tool blocks the time for the relevant participants, who can choose to accept/reject. If rejected, the tool informs the creator. It also hides participant info, so that peoples contact info can’t be seen.
5. The tool puts the link to the event on the Github thread mentioned by the creator.
6. The tool also creates a video conference event for the same, and records the event.
7. Once the event is over, the video is auto-uploaded to a publicly accessible location.
8. The tool then comments on the same ticket where the event was first shared, with the link to the recording of the event.
...
2. Calendly - Doesn’t work since it requires you to share email addresses, and there’s a paid plan for group meeting coordination.
3. Indico - Slightly cumbersome creation process, also doesn’t seem to have invite via Github ID. In addition, needs to be self-hosted.
4. Rallly - Can only help identify a date that works for everyone, not a time.
5. Doodle - Paid software.
